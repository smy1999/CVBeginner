...
#     keras.layers.Lambda(lambda x: tf.expand_dims(x, axis=-1), input_shape=[None]),  # 自定义层实现数据切片,增加最后一层维度
#     keras.layers.SimpleRNN(100, return_sequences=True),  # Input sequence and output sequence
#     keras.layers.SimpleRNN(100),  # Input sequence but output vector
#     keras.layers.Dense(1),
#     keras.layers.Lambda(lambda x: x * 200.0)  # 增大输出,以增强网络稳定性,或采用标准化输入的方式
# ])
...
# #  checkpoint保留最好的validation效果的位置
...
#
# model = keras.models.load_model('my_checkpoint')  # 读取最佳存档点
#
